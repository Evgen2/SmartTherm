...
/* Вернуть элемент буфера и не освободить место */
/* порядок использования:
	N = GetLbuf() //сколько элементов в буфере
 	StartRead();  //запоминаем ibuf2 
	Read(); // читаем элемент и продвигаем позицию ibuf2 не более N раз
	EndRead();    // освобождаем место ibuf = ibuf2
 */
void myBuffer2::Read(void *ptr)
{   
   memcpy(ptr,(void *)&pbuf[ibuf2], Litem);
   ibuf2 += Litem;	
    if(ibuf2 == Lbuf)
			     ibuf2 = 0;
}
